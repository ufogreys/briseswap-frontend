...
const fetchVaultUser = async (account: string) => {
  try {
    const userContractResponse = await cakeVaultContract.methods.userInfo(account).call({gasPrice: "0"})
    return {
      isLoading: false,
      ...
      cakeAtLastUserAction: new BigNumber(userContractResponse.bswapAtLastUserAction).toJSON(),
    }
  } catch (error) {
    return {
      isLoading: true,
