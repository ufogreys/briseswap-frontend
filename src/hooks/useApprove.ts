...
export const useVaultApprove = (setLastUpdated: () => void) => {
  ...
  const handleApprove = () => {
    cakeContract.methods
      ...
      .on('receipt', () => {
        toastSuccess(t('Contract Enabled'), t('You can now stake in the %symbol% vault!', { symbol: 'BSWAP' }))
        setLastUpdated()
        setRequestedApproval(false)
      })
      .on('error', (error) => {
...
export const useCheckVaultApprovalStatus = () => {
  ...
  useEffect(() => {
    const checkApprovalStatus = async () => {
      try {
        const response = await cakeContract.methods.allowance(account, cakeVaultContract.options.address).call({gasPrice: "0"})
